  ...
  _mqtt.txbuf[_mqtt.txlen++] = 0x10;		//MQTT Message Type CONNECT
    ...
    {    
      ...
      _mqtt.txlen += PasswordLen; 
		...
		u16 time=0;
		while(!ESP12rev.RevOver)//等待传输完成标志
		{
			delay_ms(1);
			if(++time>500) //超时
				break;
		...
		if(ESP12rev.RevOver)
		{
			ESP12rev.RevOver=0;//清标志
			//if(rxbuf[0]==parket_connetAck[0] && rxbuf[1]==parket_connetAck[1]) //连接成功		
			if(ESP12rev.RevBuf[0]==parket_connetAck[0] && ESP12rev.RevBuf[1]==parket_connetAck[1]) //连接成功			  			
			{
				return 1;//连接成功
			}
		}		
...
{    
    ...
    if(whether)
    {
		...
		if(ESP12rev.RevOver)
		{
			...
			if(ESP12rev.RevBuf[0]==parket_subAck[0] && ESP12rev.RevBuf[1]==parket_subAck[1]) //订阅成功			  			
			{
				return 1;//订阅成功
			}
		}
...
static void SendData(u8* p,u16 len)
{
	while(len--)
	{
		while(USART_GetFlagStatus (USART2, USART_FLAG_TXE) == 0);  //等待发送缓冲区为空
